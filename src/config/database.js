...
const isNeonLocalHost = ['localhost', '127.0.0.1', 'neon-local'].includes(parsedDatabaseUrl.hostname)

if (isNeonLocalHost) {
	const fetchHost = parsedDatabaseUrl.hostname
	const fetchPort = parsedDatabaseUrl.port || '5432'

	neonConfig.fetchEndpoint = process.env.NEON_FETCH_ENDPOINT || `http://${fetchHost}:${fetchPort}/sql`
